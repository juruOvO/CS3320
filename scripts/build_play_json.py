...
SRC_DIR = ROOT / "京剧剧本"
OUT_DIR = ROOT / "京剧剧本_json"
SPEC = SRC_DIR / "数据说明.xlsx"
...
def load_collection_names() -> dict[str, str]:
    if not SPEC.exists():
        return {}
...
def main():
    ...
    coll_names = load_collection_names()
    ...
    for i, pdf in enumerate(pdfs, 1):
        rel_dir = pdf.parent.name  # e.g. "01000000"
        ...
        if err and not play_json:
            failures.append((str(pdf.relative_to(SRC_DIR)), err))
            continue
        try:
            out_path.write_text(
            ...
            failures.append((str(pdf.relative_to(SRC_DIR)), f"write_failed: {e!r}"))
    ...
    if failures:
        ...
        for f, r in failures[:20]:
            report.append(f"  {f}: {r}")
    (OUT_DIR / "_parse_report.txt").write_text("\n".join(report), encoding="utf-8")
